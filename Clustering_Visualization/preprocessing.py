...
def preprocess_dataset(
    ...
    imp_strategy_numerical:str ="median",
    imp_strategy_categorical: str="KNNImputer",
    skip_cols: list[str] = ["class", "a17", "clase"], # 
    ...
    numerical_columns = [col for col in df.select_dtypes(include=["number"]).columns.tolist()]
...
def arff_to_pd(path: str, col_names: list[str]=None) -> pd.DataFrame:
    ...
    return df.loc[ : , df.columns != df.columns[-1]] # return all except the last column
...
def parse_dataset(dataset_name: str) -> pd.DataFrame:
    ...
    return arff_to_pd(dataset_name)
